...
P_=zpk(1.47*(s+8.71)*(s-8.71)/s^2) %beta to alpha transfer funtion.
...
P_=T2*P_
k1 = -0.2; %-2 %-0.2;
%Cc1 = zpk(k1*(s-P_.p{1}(1))*(s-P_.p{1}(2))*(s+0.1)^2/(s*(s+100)^3));
Cc1 = zpk(k1*(s-P_.p{1}(1))*(s-P_.p{1}(2))*(s+0.1)^2/(s*(s+P_.z{1}(1))*(s+100)^2));
%Cc1 = zpk(k1*(s-10)*(s+1.002)/((s+8.71)*(s+4/Ts)));
L1 = minreal(Cc1*P_)
figure;
bode(L1);
title('bode(L1)');
...
wc = 54; %maximum sensitivity frequency (measured using bode(S)).
Cd2 = c2d(Cc2,Ts,c2dOptions('Method','tustin','PrewarpFrequency',wc));
[A2, B2, C2, D2] = ssdata(Cd2);
wc = 3.91; %maximum sensitivity frequency (measured using bode(S)).
