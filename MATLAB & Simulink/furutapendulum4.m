...
%cell2mat(P.p)
Pmp = zpk([],[Pp(2) Pp(2)],Pk); %P.z{1} = cell2mat(P.z)
Pap = zpk(-Pp(1),Pp(1),1); %P.p{1} = cell2mat(P.p)
...
k2 = 150; %390;%660; %230e3;150e3;
Cc2 = zpk(-Pp(1),[],1)*zpk([],-100,k2); %PD
%Cc2 = zpk(k2*s*(s+P.p{1}(1))/((s-1)*(s+4/Ts)));
%Cc2 = zpk(k2*s*(s+P.p{1}(1))/((s-1)*(s+4/Ts)^2));
...
%% Secundary Loop Trasnfer Function.
% pade.u='u';
% pade.y='u_';
% P=zpk(ss(A,B,[[1 0 0 0];C],[D;0])) %gives 2 transfer function (1st: u->alpha, 2nd u->beta).
% P.u='u_';
% P.y='y';
% P1=ss([],[],[],[1 0]); %to obtain alpha.
% P1.u='y';
% P1.y='alpha';
% P2=ss([],[],[],[0 1]); %to obtain beta.
% P2.u='y';
% P2.y='beta';
% Cc2.u='beta';
% Cc2.y='u2'
% sum=sumblk('u=u1-u2');
% P_=zpk(connect(pade,P,P1,P2,Cc2,sum,'u1','alpha'))
% [P_z,P_p,P_k]=zpkdata(P_,'v');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[A2, B2, C2, D2] = ssdata(Cc2);
P_=zpk(ss([A-B*D2*C -B*C2;B2*C A2],[B;zeros(size(A2,1),1)],[[1 0 0 0] zeros(1,size(A2,1))],D))
[P_z,P_p,P_k]=zpkdata(P_,'v');

%% Loop shaping (Cascade control - Primary control).
k1 = 0.038; %0.038; %0.3; %2.8e-3;
Cc1 = zpk([P_p(3) P_p(4) P_p(5)],[P_z(1) P_z(3)],1)*zpk([-0.1 -0.1],[0 -100 -100],-k1)
%Cc1 = zpk([P_p(3) P_p(4) P_p(5)],P_z(1),1)*zpk([-0.1 -0.1],[0 -100 -100 -100],-k1);
%Cc1 = zpk([P_p(3) P_p(4) P_p(5)],[P_z(1) P_z(3)],1)*zpk([-0.1 -0.1],[0 -10 -100],-k1);
L1 = minreal(Cc1*P_)
figure;
bode(L1);
title('bode(L1)');
...
wc = 58.1; %maximum sensitivity frequency (measured using bode(S)).
Cd2 = c2d(Cc2,Ts,c2dOptions('Method','tustin','PrewarpFrequency',wc));
[A2, B2, C2, D2] = ssdata(Cd2);
wc = 10.6; %maximum sensitivity frequency (measured using bode(S)).
%wc = 61.3; %maximum sensitivity frequency (measured using bode(S)).
Cd1 = c2d(Cc1,Ts,c2dOptions('Method','tustin','PrewarpFrequency',wc));
[A1, B1, C1, D1] = ssdata(Cd1);
